...
def main():
    ...
    actions_arg_spec = dict(operation=dict(type='str', choices=['create',
                                                'destroy',
                                                'update',
                                                'claim',
                                                'bind',
    ...
    argument_spec.update(state=dict(type='str', choices=['present', 'query', 'absent'], default='present'),
                         ...
                         actions=dict(type='list', default=None, elements='dict', options=actions_arg_spec),
                         )
    ...
    query_one_urls = {'action_batch': '/organizations/{org_id}/actionBatches/{action_batch_id}'}
    create_urls = {'action_batch': '/organizations/{org_id}/actionBatches'}
    update_urls = {'action_batch': '/organizations/{org_id}/actionBatches/{action_batch_id}'}
    delete_urls = {'action_batch': '/organizations/{org_id}/actionBatches/{action_batch_id}'}
    ...
    if meraki.params['state'] == 'query':
        if meraki.params['action_batch_id'] is None:  # Get all Action Batches
            ...
            path = meraki.construct_path('get_one', org_id=org_id, custom={'action_batch_id': meraki.params['action_batch_id']})
            response = meraki.request(path, method='GET')
            if meraki.status == 200:
                meraki.result['data'] = response
                meraki.exit_json(**meraki.result) 
        ...
        if meraki.params['action_batch_id'] is None:  # Create a new Action Batch job
            ...
            payload = _construct_payload(meraki)
            if meraki.params['actions'] is not None:  # Cannot update the body once a job is submitted
                    meraki.fail_json(msg="Body cannot be updated on existing job.")
            if meraki.is_update_required(current, payload) is True:  # Job needs to be modified
                path = meraki.construct_path('update', org_id=org_id, custom={'action_batch_id': meraki.params['action_batch_id']})
        ...
        path = meraki.construct_path('delete', org_id=org_id, custom={'action_batch_id': meraki.params['action_batch_id']})
